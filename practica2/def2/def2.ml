...
let p =  let pi = 2.0 *. asin 1.0 in function r -> if r > 0.0 then 2.0 *. pi *. r else 0.0;;
...
let area = let pi = 2.0 *. asin 1.0 in function x-> if x>0. then pi *. x *. x else 0.0;;


(* Una función absf : float -> float que haga correponder a cada número su valor
absoluto *)

let absf = function (x: float) -> abs_float x;;
...
let even = function x -> x mod 2 = 0;; (*si el resto de x es 2 devuelve true, sino devuelve false *)
...
(* Y por último, redefina la función string of bool : bool -> string, de modo que devuelva (adecuadamente) los valores "verdadero" o "falso" (puede hacerse una definición por casos o utilizarse una expresión if-then-else *)
